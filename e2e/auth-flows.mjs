...
try {
  ...
  await p1.getByRole('button', { name: 'Join the family' }).click()
  ...
  await p2.getByRole('button', { name: 'Join the family' }).click()
  ...
  await expectText(p1, 'Start The Night', { label: 'auth host controls' })
  step('signed-up user created a room and is host')

  await p1.reload()
  await expectText(p1, 'Start The Night', { label: 'auth host after refresh' })
  ...
  await p3.getByRole('button', { name: 'Enter the village' }).click()
